      ...
      <div class="grid grid-cols-1 gap-4">
        ...
        <div class="flex gap-x-3 w-full">
          <div class="flex flex-col gap-1 w-full">
            <label for="status" class="font-normal text-gray-700">
              Статус
            </label>
            <select
              id="status"
          ...
          <div class="flex flex-col gap-1 w-full">
            <label for="priority" class="font-normal text-gray-700">
              Приоритет
            </label>
            <select
              id="priority"
...
function createTask() {
  ...
  employeesService.createTaskRequest(task).then(() => {
    tasksStore.getTasks();
    query.value = {};
  });
}
